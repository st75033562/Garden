...
namespace ExitGames.Client.Photon.Voice
{
    ...
    internal class RemoteVoice : IDisposable
    {
        ...
        {
            const byte MaxDiff = byte.MaxValue / 2;

            // check if the received event number is newer
            if ((byte)(evNumber - this.lastEvNumber) >= MaxDiff)
            {
                return;
            }
...
//            this.client.DebugReturn(DebugLevel.INFO, "[PV]Decode: " + res.Length /* *4 */ + "/" + buffer.Length + " " + Util.tostr(res) + " " + Util.tostr(buffer));
    ...
    public static class VoiceUtil
    {
        ...
        public static void Resample(float[] src, float[] dst, int channels)
        {
    ...
    public class LevelMeter
    {
        ...
        public LevelMeter(int samplingRate, int numChannels)
        {
        ...
        {
            this.ampPeak = 0;
            this.prevValuesPtr = 0;
            this.CurrentPeakAmp = 0;
