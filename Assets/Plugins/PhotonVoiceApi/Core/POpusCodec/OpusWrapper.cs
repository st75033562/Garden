...
//        private static extern int opus_decode_float(IntPtr st, IntPtr data, int len, float[] pcm, int frame_size, int decode_fec);
        ...
        public static int opus_decode(IntPtr st, byte[] data, int length, float[] pcm, int decode_fec, int channels)
        {
            if (st == IntPtr.Zero)
                throw new ObjectDisposedException("OpusDecoder");
            ...
            if (data != null)
            {
                numSamplesDecoded = opus_decode_float(st, data, length, pcm, pcm.Length / channels, decode_fec);
            }
            else
            {
