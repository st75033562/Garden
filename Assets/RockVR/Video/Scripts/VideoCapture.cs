...
namespace RockVR.Video
{
    ...
    public class VideoCapture : VideoCaptureBase
    {
        ...
        public override void StartCapture()
        {
            ...
            if (mode == ModeType.LOCAL && string.IsNullOrEmpty(filePath))
            {
            ...
            {
                if ((stereo == StereoType.NONE || format == FormatType.NORMAL) && isDedicated ||
                    format == FormatType.PANORAMA && panoramaProjection == PanoramaProjectionType.EQUIRECTANGULAR)
                {
            ...
            if (offlineRender)
            {
                ...
                Time.maximumDeltaTime = Time.fixedUnscaledDeltaTime;
            }
        ...
        private void LateUpdate()
        {
            if (status != VideoCaptureCtrl.StatusType.STARTED || // Capture not started yet.
                status == VideoCaptureCtrlBase.StatusType.PAUSED)// Whether is pausing.
                return;
            capturingTime += Time.unscaledDeltaTime;
            if (!isCapturingFrame)
            {
    ...
    public class VideoMuxing
    {
        ...
        public void Muxing()
        {
            ...
            using (var proc = new Process())
            {
                ...
                string[] args = {
                                    "-i", Utils.QuoteArgument(videoCapture.filePath),
                                    "-i", Utils.QuoteArgument(audioCapture.filePath),
                                    "-ar 44100 -preset ultrafast -y",
                                    "-loglevel warning",
                                    Utils.QuoteArgument(filePath)
                                };
                ...
                proc.ErrorDataReceived += (sender, eventArgs) => {
                    // ffmpeg log to stderr, so don't LogError
                ...
                if (proc.ExitCode != 0)
                {
                    throw new ApplicationException("failed to with exit code: " + proc.ExitCode);
                }
            }
        }
