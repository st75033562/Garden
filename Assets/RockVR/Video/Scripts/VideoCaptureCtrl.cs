...
namespace RockVR.Video
{
    ...
    public class VideoCaptureCtrl : VideoCaptureCtrlBase
    {
        ...
        public new static VideoCaptureCtrl instance
        {
            get { return VideoCaptureCtrlBase.instance as VideoCaptureCtrl; }
        }

        /// <summary>
        /// return the paths to last successfully recorded videos, not thread safe, call after FINISH
        /// </summary>
        public string[] lastVideoPaths
        {
            get;
            private set;
        }
        ...
        public override void StartCapture()
        {
            ...
            for (int i = 0; i < videoCaptures.Length; i++)
            {
                ...
                videoCapture.filePath = Path.Combine(FileUtils.appTempPath, StringUtils.GetRandomMp4FileName());
            ...
            if (IsCaptureAudio())
            {
                audioCapture.filePath = Path.Combine(FileUtils.appTempPath, StringUtils.GetRandomWavFileName());
        ...
        private void OnVideoCaptureComplete()
        {
                ...
                !isCaptureAudio)// No audio capture required.
            {
                ...
                // move all temporary video files to the place
                foreach (var capture in videoCaptures)
                {
                    try
                    {
                        ...
                        {
                            MoveVideoToTarget(videoPaths, capture.filePath);
                        }
                    }
                    catch (Exception e)
                    {
                        Debug.LogErrorFormat("Failed to move {0} to {1}", capture.filePath, PathConfig.saveFolder);
                        Debug.LogException(e);

                        if (eventDelegate.OnError != null)
                            eventDelegate.OnError((int)ErrorCodeType.VIDEO_IO_ERROR);
                    }
                }

                lastVideoPaths = videoPaths.ToArray();
                status = StatusType.FINISH;
        ...
        /// <summary>
        /// move the video to the target directory, add the target path to the videoPaths if no error occurs
        /// </summary>
        private void MoveVideoToTarget(List<string> videoPaths, string videoPath)
        {
            var target = PathConfig.saveFolder + Path.GetFileName(videoPath);
            File.Move(videoPath, target);
        ...
        private void VideoMergeThreadFunction()
        {
            ...
            foreach (VideoCapture videoCapture in videoCaptures)
            {
                ...
                muxing.filePath = Path.Combine(FileUtils.appTempPath, StringUtils.GetRandomMp4FileName());
                try
                {
                    muxing.Muxing();
                    MoveVideoToTarget(videoPaths, muxing.filePath);
                }
                catch (Exception e)
                {
                    Debug.LogError("muxing failed");
                    Debug.LogException(e);
