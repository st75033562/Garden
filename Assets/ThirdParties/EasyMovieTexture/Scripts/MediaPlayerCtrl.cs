...
public class MediaPlayerCtrl : MonoBehaviour
{
	...
	private unsafe bool Call_Load(string strFileName, int iSeek)
	{
		...
		loader = new Thread(() =>
		{
            ...
            catch (Exception e)
            {
                ...
                AddActionForUnityMainThread(() => {
                    OnError(MEDIAPLAYER_ERROR.MEDIA_ERROR_IO, MEDIAPLAYER_ERROR.MEDIA_ERROR_IO);
                });
            }
		});
							...
							if (gotSound == 1)
							{
								...
								if (pAudioCodecContext->sample_fmt != AVSampleFormat.AV_SAMPLE_FMT_FLT)
								{
									//for(int i = 0; i < pAudioCodecContext->channels; i++)
									{
										...
										System.Buffer.BlockCopy(buffer, 0, fAudioData, 0, buffer.Length);
	...
	private void Call_UpdateVideoTexture()
	{
		...
		if (m_CurrentState == MEDIAPLAYER_STATE.PLAYING && m_bIsFirstFrameReady == true && bInterrupt == false)
		{
			if (listVideo.Count > 0)
				fCurrentSeekTime += Time.unscaledDeltaTime * m_fSpeed;
		}
		...
		{
            // find the latest frame to display
            byte[] frame = null;
            float fpts = -1;
            while (listVideoPts.Count > 0 && fLastFrameTime <= fCurrentSeekTime - 0.1f)
            {
                frame = listVideo.Dequeue();
                fpts = listVideoPts.Dequeue();
...

			{
				...
				if (frame != null)
				{
					SetTextureFromUnity(m_iID, m_texPtr, m_iWidth, m_iHeight, frame);
            ...
            if(frame != null)
            {
                    m_VideoTexture.LoadRawTextureData(frame);
	...
	audioClip = AudioClip.Create("videoAudio",(int)((float)pAudioCodecContext->sample_rate * 600.0f),pAudioCodecContext->channels,pAudioCodecContext->sample_rate,false,false);
			...
			}//while(fLastFrameTime < fCurrentSeekTime - 0.1f);
	...
	private int Call_GetSeekPosition()
	{
        ...
        return (int)(fLastFrameTime * 1000.0f);
	}
