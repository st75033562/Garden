...
//			PBXDictionary results = new PBXDictionary();
            ...
            if (sourceDirectoryInfo.Name.EndsWith(".lproj"))
            {
                System.Uri fileURI = new System.Uri(folderPath);
                System.Uri rootURI = new System.Uri((projectRootPath + "/."));
...
//				UnityEngine.Debug.Log( "--> " + file + ", " + newGroup );
        ...
        {
            foreach (var current in fileReferences.Values) {
                if (current.filePath.Equals(path, StringComparison.Ordinal)) {
                    return current;
                }
            }
            return null;
